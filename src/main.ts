...
root.innerHTML = `
  ...
  <div id="scaleViewport" class="scale-viewport">
    <main id="scaleContent" class="shell">
...
const updateScale = (): void => {
  const width = scaleViewport.clientWidth;
  const height = scaleViewport.clientHeight;
  if (width <= 0 || height <= 0) {
    return;
  }

  const scale = Math.min(width / DESIGN_WIDTH, height / DESIGN_HEIGHT);
  scaleContent.style.transform = `scale(${Math.max(0.1, scale)})`;
};

const scaleObserver = new ResizeObserver(() => {
  updateScale();
});
scaleObserver.observe(scaleViewport);
updateScale();
